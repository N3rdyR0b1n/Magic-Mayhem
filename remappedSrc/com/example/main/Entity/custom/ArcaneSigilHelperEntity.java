...
public class ArcaneSigilHelperEntity extends ThrownItemEntity {
    ...
    public void tick() {
        ...
        if (GetEffect() == null) {
            this.method_48926().addImportantParticle(ParticleTypes.FLAME, true, this.getX(), this.getY(), this.getZ(), 0, 0.5, 0);
            return;
        }
        if (this.age % 3 == 0) {
            float silly = this.age;
            silly /= 6.283f;
            this.method_48926().addImportantParticle(GetEffect(), true, (MathHelper.sin(silly) * getExplosionRange()) + this.getX(), this.getY() + 0.6f, (MathHelper.cos(silly) * getExplosionRange()) + this.getZ(), 0, 0.05, 0);
        ...
        super.tick();
        if (this.age == -2 && !this.method_48926().isClient) {
            this.kill();
        }
    }
    ...
    private boolean shouldExplode(float range) {
        ...
        List<LivingEntity> Entities = this.method_48926().getNonSpectatingEntities(LivingEntity.class, this.getBoundingBox().expand(range));
        for (LivingEntity entity : Entities) {
    ...
    public void explode(float areamultiplier, PlayerEntity player, float knockbackmultiplier, float weak, float strong) {
        ...
        List<LivingEntity> list = this.method_48926().getNonSpectatingEntities(LivingEntity.class, shooter.getBoundingBox().expand(5.0 * areamultiplier));
        for (LivingEntity livingEntity : list) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                BlockHitResult hitResult = this.method_48926().raycast(new RaycastContext(vec3d, vec3d2, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, shooter));
                if (((HitResult) hitResult).getType() != HitResult.Type.MISS) continue;
                bl = true;
                break;
            }
            ...
            if (aa != 0.0) {
                ...
                double ac = (1.0 - w) * ab;
                if (bl) {
                    ModSpells.ARCANE_SIGIL.onHit(player, this.method_48926(),livingEntity, weak);
                    weakhit(livingEntity);
                    if (livingEntity.isDead()) {
                        ModSpells.ARCANE_SIGIL.onKill(player, this.method_48926(), livingEntity);
                    }
                }
                else {
                    ModSpells.ARCANE_SIGIL.onHit(player, this.method_48926(),livingEntity, strong);
