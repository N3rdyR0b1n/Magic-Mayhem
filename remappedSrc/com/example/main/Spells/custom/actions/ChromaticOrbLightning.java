            ...
            ChromaticOrbEntity orb = new ChromaticOrbEntity(ModEntities.CHROMATIC_ORB, player, world, stack) {
                ...
                protected void onEntityHit(EntityHitResult entityHitResult) {
                    if (this.getOwner() instanceof  PlayerEntity owner) {
                        ...
                        List<LivingEntity> entities = this.GetNearbyEntities(5f, this.method_48926(), owner);
                        Random random = new Random();
                        for (LivingEntity target : entities) {
                            Vec3d direction = target.getPos().subtract(this.getPos());
                            direction.normalize();
                            this.method_48926().addImportantParticle(
                        ...
                        LightningEntity lightning = new LightningEntity(EntityType.LIGHTNING_BOLT, this.method_48926());
                        lightning.setPosition(this.getPos());
                        this.method_48926().spawnEntity(lightning);
                    }
                    this.HasHit();
                }
