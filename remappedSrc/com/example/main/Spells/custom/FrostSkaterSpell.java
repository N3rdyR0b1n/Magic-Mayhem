...
public class FrostSkaterSpell extends ContinousUsageSpell {
    ...
    public void tickAction(PlayerEntity player, World world, ItemStack stack, int index, int tick) {
        int level = Level();
        FrostWalkerEnchantment.freezeWater(player, player.method_48926(), player.getBlockPos(), 2 + level);
        if (player.isOnGround() && player.getSteppingBlockState().getBlock() instanceof IceBlock) {
            if (player.isInPose(EntityPose.CROUCHING)) {
                ...
                movement = new Vec3d(movement.x, 0, movement.z).normalize().multiply(1.25f + 0.1f * level);
                ...
                if (explode.x != 0 || explode.z != 0) {
                    if (GenericSpellAbilities.explodeFlatAoe(15 + level * 5,
                            2, 0.25f, world, player, player, this, 1f, 0.5f).size() > 1) {
                        player.setVelocity(movement.multiply(3).add(0,0.25 + level * 0.125f, 0));
