...
public class CommandGrabSpell extends ContinousUsageSpell {
    ...
    public CommandGrabSpell(int manaCost, SpellSchool school, int chargeTime, String name, int cooldown, Identifier texture, int useTime, boolean needsHeld, int overtimecost, int levels, int maxlevel, int upcost) {
        super(manaCost, school, chargeTime, name, cooldown, texture, useTime, needsHeld, overtimecost, levels, maxlevel, upcost, 0);
        text = ModSpells.FormatSpell(name, manaCost, overtimecost * 4, level);
    }
    ...
    public void castSpell(PlayerEntity player, World world, ItemStack stack) {
        ...
        if (!world.isClient()) {
            ...
            GravityHelperEntity gravity = new GravityHelperEntity(player, world, entity, Level());
    ...
    public void applyCooldown(PlayerEntity player, ItemStack stack, NbtCompound nbt, int slot) {
        if (!player.getWorld().isClient() && nbt.getBoolean("Hit")) {
            UseTime = -1;
