...
public class FireBallSpellEntity extends ThrownItemEntity {
    ...
    private void explode(Vec3d pos) {
        if (hittime == -1) {
            ...
            this.getWorld().playSound(null, this.getBlockPos(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS, 10, 1);
            this.setPosition(pos);
            if (this.getOwner() instanceof PlayerEntity player) {
                GenericSpellAbilities.explodeFlatAoe(damage, knockback, radius, this.getWorld(), this, player, ModSpells.FIREBALL, stronghit, weakhit);
            }
